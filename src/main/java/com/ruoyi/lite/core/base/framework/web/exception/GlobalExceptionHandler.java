package com.ruoyi.lite.core.base.framework.web.exception;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<AjaxResult> handleNoResource(NoResourceFoundException e, HttpServletRequest request) {
        String path = e.getResourcePath();
        if (path.endsWith(".map")) {
            // Source Map 文件找不到，静默返回 404，不打日志
            return ResponseEntity.status(404).body(AjaxResult.error("Not Found"));
        }
        
        // 其他资源异常正常走
        log.error("资源未找到: {}", request.getRequestURI(), e);
        return ResponseEntity.status(404).body(AjaxResult.error("资源不存在"));
    }
